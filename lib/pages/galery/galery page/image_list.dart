...
class PortafolioImages extends StatefulWidget {
  ...
  const PortafolioImages(
      {super.key, required this.snapshot, required this.galeryType});

  @override
  State<PortafolioImages> createState() =>
      _PortafolioImagesState(snapshot: snapshot, galeryType: galeryType);
}
...
class _PortafolioImagesState extends State<PortafolioImages> {
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < dataLength!; i++) {
      images.add(
        ImageManager(
            imageSelected: data?[i], indexSelected: i, galeryType: galeryType),
      );
    }
